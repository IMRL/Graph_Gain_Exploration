...
LidarIris::FeatureDesc LidarIris::UpdateFrame(const cv::Mat1b &frame, int frameIndex)
{
    auto feature = GetFeature(frame);
    feature.index = frameIndex;
    flann::Matrix<float> queries(feature.vec.data(), 1, feature.vec.size());
...
void LidarIris::Compare(LidarIris::FeatureDesc &frame, int *matchIndex, float *matchDis, int *matchBias)
{
    flann::Matrix<float> queries(frame.vec.data(), 1, frame.vec.size());
    if (featureList.size() == 0 || featureList.size() == 1 && featureList[0].index == frame.index)
    {
        if (matchIndex)
            *matchIndex = -1;
        if (matchDis)
    ...
    else
    {
        ...
        std::vector<float> dis(count);
        std::vector<int> bias(count);
        for (int i = 0; i < count; i++)
        {
            if (std::abs(featureList[indices[0][i]].index - frame.index ) < 50)
            {
                dis[i] = INFINITY;
                continue;
            }
            dis[i] = Compare(frame, featureList[indices[0][i]], &bias[i]);
        }
        // std::cout << "i am in compare!" << std::endl;
        int minIndex = std::min_element(dis.begin(), dis.end()) - dis.begin();
        if (matchIndex)
            *matchIndex = featureList[indices[0][minIndex]].index;
        if (matchDis)
            *matchDis = dis[minIndex];
        if (matchBias)
            *matchBias = bias[minIndex];
    }
}

float LidarIris::Compare(const LidarIris::FeatureDesc &img1, const LidarIris::FeatureDesc &img2, int *bias)
{
    // assert(img2.img.size() == img1.img.size());
...
LidarIris::FeatureDesc LidarIris::GetFeature(const cv::Mat1b &src)
{
    ...
    desc.vec = temp.isContinuous() ? temp : temp.clone();
    return desc;
}
