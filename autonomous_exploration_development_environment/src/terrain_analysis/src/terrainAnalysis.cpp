...
pcl::PointCloud<pcl::PointXYZI>::Ptr
    chunkmapTerrain(new pcl::PointCloud<pcl::PointXYZI>());
...
void chunkCloudHandler(const sensor_msgs::PointCloud2ConstPtr &msg) {
  pcl::fromROSMsg(*msg, *chunkmapTerrain);
}
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber subChunkCloud = nh.subscribe<sensor_msgs::PointCloud2>(
      "/chunkmap_terrain_map", 5, chunkCloudHandler);
  ...
  while (status) {
    ...
    if (newlaserCloud) {
      ...
      *terrainCloudElev += *chunkmapTerrain;
