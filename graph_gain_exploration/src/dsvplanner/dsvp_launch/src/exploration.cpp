...
ros::Publisher submap_signal_pub;
...
int main(int argc, char** argv)
{
  ...
  submap_signal_pub = nh.advertise<std_msgs::Empty>("/next_submap", 1);
    ...
    if (!return_home)
    {
      ...
      if (ros::service::call("drrtPlannerSrv", planSrv))
      {
        ...
        {  // when not in simulation mode, the robot will go to
          ...
          previous_odom_y = current_odom_y;
          previous_odom_z = current_odom_z;
          if(planSrv.response.mode.data == 4)
            ROS_WARN_STREAM("gp_in_progress = " << gp_in_progress);
          bool gpflag = gp_in_progress;
          ...
          gp_command_pub.publish(graph_planner_command);
          ROS_WARN_STREAM("planSrv.response.mode.data = " << planSrv.response.mode.data);
          if(planSrv.response.mode.data == 4 && gpflag == 1) {
              std_msgs::Empty nextSubmap;
              submap_signal_pub.publish(nextSubmap);
              ROS_WARN_STREAM("Next Submap!");
          }
        }
        else
