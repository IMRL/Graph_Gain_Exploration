...
void DualStateFrontier::getUnknowPointcloudInBoundingBox(const StateVec& center, const StateVec& bounding_box_size)
{
  ...
  for (double x_position = bbx_min.x(); x_position <= bbx_max.x(); x_position += kFrontierResolution)
  {
    for (double y_position = bbx_min.y(); y_position <= bbx_max.y(); y_position += kFrontierResolution)
    {
      ...
      // if (node == NULL)
      if (ChunkmapGetCellStatusPoint(chunk_map_, point.cast<double>()) == ChunkCellStatus::UNKNOWN)
      {
...
bool DualStateFrontier::frontierDetect(Eigen::Vector3f point) const
{
  float resolution = chunk_map_->resolution();
  bool effectiveFree = false;
  bool effectiveUnknown = false;
  int unknowCount = 0;
  static const std::vector<Eigen::Vector3f> offsets{{0, 1, 0}, {0, -1, 0}, {-1, 0, 0}, {1, 0, 0}};
  for (const auto & it : offsets)
  {
    auto status = ChunkmapGetCellStatusPoint(chunk_map_, (point + it * resolution).cast<double>());
    if (status == ChunkCellStatus::UNKNOWN)
      unknowCount++;
    else
    {
...
bool DualStateFrontier::FrontierInBoundry(Eigen::Vector3f point) const
{
  if (boundaryLoaded_)
  {
...
bool DualStateFrontier::inSensorRangeofGraphPoints(StateVec point)
{
  ...
  if (graphPoints_->points.size() > 0)
  {
    ...
    for (int i = 0; i < pointSearchInd.size(); i++)
    {
      ...
      if (ChunkCellStatus::OCCUPIED != ChunkmapGetVisibility(chunk_map_, node_point, point, false)/* &&
...
bool DualStateFrontier::inSensorRangeofRobot(StateVec point)
{
  ...
  if (ChunkCellStatus::FREE != ChunkmapGetVisibility(chunk_map_, robot_position_, point, false))
  {
    return false;
  }
  return true;
...
void DualStateFrontier::localFrontierUpdate(StateVec& center)
{
  ...
  for (int i = 0; i < local_frontier_->size(); i++)
  {
    ...
    if (!(kEliminateFrontiersAroundRobots && inSensorRangeofRobot(checkedPoint)) &&
        ...
        ((ChunkCellStatus::OCCUPIED != ChunkmapGetVisibility(chunk_map_, center, checkedPoint, false)/* &&
...
void DualStateFrontier::gloabalFrontierUpdate()
{
  ...
  for (int i = 0; i < size; i++)
  {
    ...
    if (ChunkCellStatus::UNKNOWN == ChunkmapGetCellStatusPoint(chunk_map_, point.cast<double>()) && frontierDetect(point))
    {
