#ifndef _MAP_BUILDER_NOLOOP_H_
#define _MAP_BUILDER_NOLOOP_H_
...
class MapBuilderNoloop
{
public:
    using Ptr = std::shared_ptr<MapBuilderNoloop>;

    virtual ~MapBuilderNoloop() {}
    ...
    std::set<ChunkMap::Index> draw_multi(std::vector<size_t> &need_update_submaps)
    {
        ...
        multi_execute<N1>(rotate_tasks); // TODO:
        //
...
protected:
    ...
    virtual void layer_generator(const ChunkMap::Index &chunk_index, const SubmapInfo * const submap, const RotatedCache *const rotated_cache) = 0;
    virtual std::set<ChunkMap::Index> draw_prepare(SubmapInfo &submap) = 0;
};
